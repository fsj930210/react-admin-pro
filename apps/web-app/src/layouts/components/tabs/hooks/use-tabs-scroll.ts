...
export function useTabsScroll(options: UseTabsScrollOptions = {}) {
	...
	const checkScrollability = () => {
		...
		setCanScrollRight(hasScrollbar && scrollLeft < scrollWidth - clientWidth - 20);
	};
	...
	useEffect(() => {
		...
		// 在 effect 中避免直接调用 setState，使用 queueMicrotask 延迟到浏览器绘制后执行
		queueMicrotask(() => checkScrollability());
