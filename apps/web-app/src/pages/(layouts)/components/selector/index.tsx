    ...
    <div className="min-h-0 flex-1">
      <DataGrid
        ...
        pagination={{
          ...
          contentClassName: "flex-nowrap justify-end [&_[data-slot=pagination-link]>span]:hidden",
        }}
        rowSelection={{
...
function TableDemo() {
  ...
  return (
    <Section
      title="DataGrid 渲染"
      description="真实表格场景接 pro DataGrid，并使用 DataGrid 自己的分页。"
    >
      <Selector<User>
        className="h-[520px]"
        dataSource={users}
  ...
  return (
    <div className="min-h-0 flex-1 overflow-auto rounded-md border p-2">
      <ProTree
        ...
        labelRender={(item) => (
          <span className="block max-w-[520px] truncate">{item.node.label}</span>
        )}
      />
    </div>
  );
...
function InternalPaginationDemo() {
  ...
  return (
    <Section
      title="Selector 内部分页"
      description="分页作为 Selector 内部 footer，而不是放到 Selector 外面。"
    >
