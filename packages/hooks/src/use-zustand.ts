...
type SelectorParam<T extends object> = keyof T | ReadonlyArray<keyof T> | StoreSelector<T, unknown>;

type SelectorResult<T extends object, S> =
  S extends StoreSelector<T, infer R>
    ? R
...
export type StoreSet<T extends object> = <K extends keyof T>(key: K, value: T[K]) => void;
...
export type StoreUpdateState<T extends object> = (recipe: (draft: Draft<T>) => void) => void;

export type SetterFunction<T extends object, K extends keyof T> = (
  nextValue: T[K] | ((draft: Draft<T[K]>, state: Draft<T>) => void)
) => void;

function smartEqual<T>(prev: T, next: T, customEqual?: EqualityFn<T>): boolean {
  if (customEqual) {
...
export function createStore<T extends object>(
  ...
  function use<S extends SelectorParam<T>>(
    ...
    return useStoreWithEqualityFn(store, buildSelector(selector), (prev, next) =>
      smartEqual(prev, next, equalityFn)
    );
  }
