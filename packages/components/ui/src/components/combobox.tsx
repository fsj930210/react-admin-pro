...
function ComboboxList({ className, ...props }: ComponentProps<typeof CommandPrimitive.List>) {
  return (
    <CommandPrimitive.List
      ...
      className={cn(
        "max-h-[min(calc(var(--spacing)_*_16_*_6),calc(var(--available-height)_-_var(--spacing)_*_9))] scroll-py-1 overflow-y-auto p-1 data-empty:p-0",
        className
      )}
      {...props}
    />
...
function ComboboxChip({
  ...
  return (
    <div
      ...
      className={cn(
        "flex h-[calc(var(--spacing)_*_5.5)] w-fit items-center justify-center gap-1 rounded-sm bg-muted px-1.5 text-xs font-medium whitespace-nowrap text-foreground has-disabled:pointer-events-none has-disabled:cursor-not-allowed has-disabled:opacity-50 has-data-[slot=combobox-chip-remove]:pr-0",
        className
      )}
      {...props}
    >
...
function ComboboxChipsInput({ className, ...props }: ComponentProps<"input">) {
  return (
    <input
      ...
      className={cn(
        "min-w-16 flex-1 rounded-sm border-0 bg-transparent p-1 outline-none focus:ring-0",
        className
      )}
      {...props}
    />
