...
export function ColumnSort<TData>({
  ...
  return (
    <When condition={column.getCanSort()}>
      <Choose>
        <When condition={Boolean(custom)}>{custom?.({ column, table })}</When>
        <Otherwise>
          <div className="relative shrink-0">
            <FloatingLayer onOpenChange={setOpen} open={open}>
              ...
              >
                <MenuButton
                  onClick={() => {
                    column.toggleSorting(false, table.options.enableMultiSort);
                    setOpen(false);
                  }}
                >
                  <ArrowUp className="size-3.5" />
                ...
                <MenuButton
                  onClick={() => {
                    column.toggleSorting(true, table.options.enableMultiSort);
                    setOpen(false);
                  }}
                >
                  <ArrowDown className="size-3.5" />
