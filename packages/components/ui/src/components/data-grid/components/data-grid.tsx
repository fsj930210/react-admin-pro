...
export function DataGrid<TData>(props: DataGridProps<TData>) {
  ...
  const content = (
    ...
    >
      <OverlayScrollbarsComponent
        ...
        options={view.overlayScrollbarsOptions}
        className={cn(
          view.scrollAreaProps.className,
          view.scrollbarClassName,
  ...
  return (
    <Sortable.Root
      items={columnOrderingDrag.sortableColumnIds}
        ...
        {({ id }) =>
          columnOrderingDrag.activeColumnId ? (
            <div className="pointer-events-none flex h-12 min-w-28 items-center gap-2 rounded-md border bg-popover px-3 text-sm font-medium text-popover-foreground shadow-lg">
              ...
              <span className="truncate">{columnOrderingDrag.activeColumnTitle ?? id}</span>
            </div>
          ) : null
        }
      </Sortable.Overlay>
