...
export function getOrderedVisibleLeafColumns<TData>(table: Table<TData>) {
  const model = createColumnOrderModel(table);
  const order = normalizeColumnOrder(table.getState().columnOrder, model);
  const orderRank = new Map(order.map((id, index) => [id, index]));
  const zoneRank: Record<Exclude<DataGridColumnPinZone, "mixed">, number> = {
    left: 0,
    center: 1,
    right: 2,
  };

  return [...table.getVisibleLeafColumns()].sort((left, right) => {
    const leftZone = model.pinZoneById.get(left.id);
    const rightZone = model.pinZoneById.get(right.id);
    const leftZoneRank = leftZone && leftZone !== "mixed" ? zoneRank[leftZone] : zoneRank.center;
    const rightZoneRank =
      rightZone && rightZone !== "mixed" ? zoneRank[rightZone] : zoneRank.center;
