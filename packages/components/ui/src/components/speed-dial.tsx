"use client";
// 文档地址 https://www.diceui.com/docs/components/radix/speed-dial
import {
  Children,
  createContext,
  isValidElement,
...
type RootElement = ComponentRef<typeof SpeedDial>;
type ContentElement = ComponentRef<typeof SpeedDialContent>;
type TriggerElement = ComponentRef<typeof SpeedDialTrigger>;
type ActionElement = ComponentRef<typeof SpeedDialAction>;
...
interface SpeedDialContextValue {
  ...
  isPointerInsideReactTreeRef: RefObject<boolean>;
...
function useSpeedDialContext(consumerName: string) {
  const context = use(SpeedDialContext);
  if (!context) {
    throw new Error(`\`${consumerName}\` must be used within \`${ROOT_NAME}\``);
  }
  return context;
...
function SpeedDial(props: SpeedDialProps) {
  ...
  const onNodeRegister = useCallback((node: NodeData) => {
    nodesRef.current.set(node.id, node);
  }, []);

  const onNodeUnregister = useCallback((id: string) => {
  ...
  const onPointerDownCapture = useCallback(
    (event: ReactPointerEvent<RootElement>) => {
  ...
  const contextValue = useMemo<SpeedDialContextValue>(
    () => ({
      getNodes,
      onNodeRegister,
      onNodeUnregister,
  ...
  return (
    <StoreContext value={store}>
      <SpeedDialContext value={contextValue}>
        <RootPrimitive
...
function SpeedDialTrigger(props: ComponentProps<typeof Button>) {
  const {
    onClick: onClickProp,
  ...
  const onClick = useCallback(
    (event: ReactMouseEvent<TriggerElement>) => {
      onClickProp?.(event);
      if (event.defaultPrevented) return;
...
const SpeedDialItemImplContext = createContext<SpeedDialItemImplContextValue | null>(null);

function useSpeedDialItemImplContext() {
  return use(SpeedDialItemImplContext);
}
...
const SpeedDialItemImpl = memo(function SpeedDialItemImpl({
  delay,
  open,
  children,
}: SpeedDialItemImplProps) {
  const contextValue = useMemo<SpeedDialItemImplContextValue>(
    () => ({ delay, open }),
    [delay, open]
  );

  return <SpeedDialItemImplContext value={contextValue}>{children}</SpeedDialItemImplContext>;
});
...
function SpeedDialContent(props: SpeedDialContentProps) {
  ...
  const transformOrigin = useMemo(() => getTransformOrigin(side), [side]);
  ...
  const onMouseEnter = useCallback(
    (event: ReactMouseEvent<ContentElement>) => {
...
const SpeedDialItemContext = createContext<SpeedDialItemContextValue | null>(null);

function useSpeedDialItemContext(consumerName: string) {
  const context = use(SpeedDialItemContext);
  if (!context) {
...
function SpeedDialItem(props: DivProps) {
  ...
  const contextValue = useMemo<SpeedDialItemContextValue>(
    () => ({ actionId, labelId }),
    [actionId, labelId]
  );
  ...
  return (
    <SpeedDialItemContext value={contextValue}>
      <ItemPrimitive
        role="none"
    ...
    </SpeedDialItemContext>
  );
}

interface SpeedDialActionProps extends Omit<ComponentProps<typeof Button>, "onSelect"> {
...
function SpeedDialAction(props: SpeedDialActionProps) {
  ...
  const actionRef = useRef<ActionElement | null>(null);
  ...
  const onClick = useCallback(
    (event: ReactMouseEvent<ActionElement>) => {
