...
interface RouteKeepAliveProps {
  animation?: false | AnimatorConfig;
  className?: string;
  fallback?: ReactNode;
...
function updateScrollSnapshot(
  ...
  return tabs.map((tab) => (isSameHref(tab.href, href) ? { ...tab, scrollSnapshot } : tab));
}

function tabsReducer(tabs: TabRouterEntry[], action: TabsAction): TabRouterEntry[] {
  if (action.type === "activate") {
    const withScroll = updateScrollSnapshot(tabs, action.scrollSnapshotHref, action.scrollSnapshot);
...
const DEFAULT_ROUTE_ANIMATION: AnimatorConfig = {
  preset: "route-forward",
  duration: 0.18,
  exitDuration: 0.12,
  distance: 18,
};
...
export function RouteKeepAlive({
  ...
  useEffect(() => {
    ...
    if (
      !activeTab ||
      isSameHref(getHrefFromLocation(activeTab.router.latestLocation), activeHref)
    ) {
      return;
    }
  ...
  const renderTab = (tab: TabRouterEntry) => {
    const isActive = isSameHref(tab.href, activeHref);
    const content = (
    ...
    return (
      <RaAnimator
        animateKey={isActive ? `${tab.id}-${tab.activityKey}` : undefined}
        className="size-full"
        config={animation}
      >
        {content}
  ...
  if (!keepAlive) {
    return (
      <div ref={containerRef} className={cn("size-full overflow-y-auto", className)}>
        {transientTab ? renderTab(transientTab) : fallback}
      </div>
    );
  }
